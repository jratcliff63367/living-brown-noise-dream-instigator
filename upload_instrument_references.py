#!/usr/bin/env python3
"""
Upload handpan instrument reference MP3s to ElevenLabs Music and cache song_ids.

Scans:
    ./ceremonies/indian/instrument-reference/handpan/*.mp3

Reads API key from:
    ./eleven-labs.txt

Writes/updates:
    ./ceremonies/indian/instrument-reference/handpan/reference-song-ids.json
...
Usage:
    python upload_instrument_references.py

Optional:
    python upload_instrument_references.py --force
        Re-upload every MP3 regardless of cache.

    python upload_instrument_references.py --limit 5
        Process only the first 5 MP3s found.

    python upload_instrument_references.py --pause 2.0
...
REFERENCE_DIR = SCRIPT_DIR / "ceremonies" / "indian" / "instrument-reference" / "handpan"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Upload handpan instrument reference MP3s to ElevenLabs and cache song_ids."
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    # Import successful uploads from the first instrument uploader, if used.
    # Keep the old file untouched; the vocal-compatible registry is authoritative.
    legacy_path = REFERENCE_DIR / "song_ids.json"
    if legacy_path.exists():
        legacy = json.loads(legacy_path.read_text(encoding="utf-8"))
        for entry in legacy.get("references", []):
            if not isinstance(entry, dict):
                continue
            name = entry.get("filename")
            if name and entry.get("song_id") and entry.get("sha256") and name not in registry["files"]:
                registry["files"][name] = {
                    "filename": name,
                    "sha256": entry["sha256"],
                    ...
                    "uploaded_utc": entry.get("uploaded_at"),
