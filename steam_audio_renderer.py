...
class SteamAudioSource:
    ...
    def __init__(
        ...
        *,
        ...
        self._current_distance_gain = self._distance_gain(
            position.distance()
        )
    ...
    def _process_mono_exact(
        ...
        if state.distance_attenuation_enabled:
            target_gain = self._distance_gain(
                state.position.distance()
            )
            gain_curve = np.linspace(
                self._current_distance_gain,
                target_gain,
                len(output),
                endpoint=True,
                dtype=np.float32,
            )
            output *= gain_curve[:, np.newaxis]
            self._current_distance_gain = float(target_gain)
        else:
            self._current_distance_gain = 1.0
    ...
    def process_mono(
        ...
        """Render an arbitrary-length mono NumPy buffer to stereo float32."""
        samples = np.asarray(mono, dtype=np.float32)
        ...
        for start in range(0, len(samples), frame_size):
            valid_count = min(frame_size, len(samples) - start)
            block = np.zeros(frame_size, dtype=np.float32)
            block[:valid_count] = samples[start:start + valid_count]
    ...
    def process_stereo_bed(
        ...
        *,
        ...
        """Blend an existing stereo bed with a mono-derived binaural render."""
        stereo32 = np.asarray(stereo, dtype=np.float32)
        ...
        amount = float(np.clip(spatial_amount, 0.0, 1.0))

        if amount <= 0.0 or len(stereo32) == 0:
            return stereo32.copy()

        mono = 0.5 * (stereo32[:, 0] + stereo32[:, 1])
        spatial = self.process_mono(mono)

        return (
            stereo32 + (spatial - stereo32) * amount
...
def _self_test() -> None:
    print("Steam Audio renderer wrapper self-test")
